defmodule Encryption.PasswordFieldTest do
  ...
  test ".load does not modify the hash, since the hash cannot be reversed" do
    hash = Field.hash("password")
    assert {:ok, ^hash} = Field.load(hash)
  end

  test "verify_pass checks the password against the Argon2id Hash" do
    password = "EverythingisAwesome"
    hash = Field.hash(password)
    verified = Field.verify_pass(password, hash)
  ...
  test ".verify_pass fails if password does NOT match hash" do
    password = "EverythingisAwesome"
    hash = Field.hash(password)
    verified = Field.verify_pass("LordBusiness", hash)
    # IO.inspect verified, label: "verified"
    assert !verified
  end
end
